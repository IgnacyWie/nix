...
OMLX, imsg, and himalaya are installed on `eta` through host-specific Homebrew
configuration. The `eta` Homebrew scope is additive: activation does not clean
up or remove unlisted Homebrew software. OMLX is the host-managed Local Model
Runtime for Local AI Service Stacks. This Homebrew scope is separate from the
`gamma` Workstation Homebrew scope. The initial model preference is configurable with
